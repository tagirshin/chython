...
cdef dist_matrix_t *alloc_dist_matrix(unsigned short n_nodes) noexcept:
...
cdef rings_t *alloc_rings(unsigned short n_rings, size_t hash_size) noexcept:
...
cdef void realloc_rings(rings_t *rings, size_t n_rings) noexcept:
    cdef size_t i
...
cdef void free_pid0(paths_t *paths) noexcept:
    cdef size_t i
...
cdef void free_pid1(paths_t *paths) noexcept:
    cdef size_t i
...
cdef void free_pids(paths_t *paths) noexcept:
    free_pid0(paths)
    free_pid1(paths)


cdef void free_dist_matrix(dist_matrix_t *matrix) noexcept:
...
cdef void free_ring(ring_t *ring) noexcept:
...
cdef void free_rings(rings_t *rings) noexcept:
    cdef size_t i
...
cdef void move_paths(paths_t *paths) noexcept:
    free_pid1(paths)
...
cdef unsigned short *concatenate_paths(paths_t *paths_ik, paths_t *paths_kj, unsigned short k) noexcept:
...
cdef void append_pid0(paths_t *paths_ij, paths_t *paths_ik, paths_t *paths_kj, unsigned short k) noexcept:
...
cdef void append_pid1(paths_t *paths_ij, paths_t *paths_ik, paths_t *paths_kj, unsigned short k) noexcept:
...
cdef int has_not_overlap(paths_t *paths_ik, paths_t *paths_kj, paths_t *paths_ij, unsigned short k, int test_pid1) noexcept:
    cdef size_t i, j
...
cdef unsigned long long *build_hash(unsigned short *path1, unsigned short *path2,
                                    unsigned short i, unsigned short j, size_t size1, size_t size2, size_t hash_size) noexcept:
    cdef size_t k
...
cdef unsigned short *build_ring(unsigned short *path1, unsigned short *path2,
                                unsigned short i, unsigned short j, size_t size1, size_t size2) noexcept:
...
cdef void push_ring(ring_t *ring, rings_t *rings, unsigned short rank) noexcept:
    cdef size_t i
...
cdef unsigned short get_rank(ring_t *ring, rings_t *rings) noexcept:
    cdef size_t i, j
    cdef int hash_match
    cdef ring_t *other
...
cdef int build_pid(dist_matrix_t *matrix, time_t deadline) noexcept:
...
cdef rings_t *build_cset(dist_matrix_t *matrix, size_t n_rings, time_t deadline) noexcept:
...
cdef void filter_rings(rings_t *rings) noexcept:
...
def sssr(object graph, size_t n_rings, double timeout=0):
    ...
    # ponytail: GIL held throughout, so concurrent threads do not overlap here. upstream 23f3bfd releases
    # it, but `nogil` forces PyMem_Raw* allocators, measured ~1.23x slower single-threaded on fused
    # polycyclics. consumers parallelise with processes, so that trade is a loss. switch to upstream's
    # `with nogil` form if a threaded consumer ever appears.
